...
function unsafeTask(text='') {
  return /(me matar|suicid|machucar alguém|matar alguém|explosiv|fraude|invadir conta|emergência médica|overdose)/i.test(text.toLowerCase());
}
...
function buildAction(level=1){
  ...
  current = {
    id: crypto.randomUUID ? crypto.randomUUID() : String(Date.now()), task, barrier,
    resistance_before: resistance, created_at: new Date().toISOString(), action_level: level,
    micro_action: actionMap[barrier](task, level), attempts: []
  };
  ...
  $('begin-btn').textContent = 'Começar agora';
...
function startTimer(){
  if (!current) return;
  current.attempts.push({started_at:new Date().toISOString(), completed_at:null, outcome:null});
  secondsLeft = current.action_level > 1 ? 120 : 300;
  renderTimer();
  clearInterval(timerHandle);
  timerHandle = setInterval(()=>{ secondsLeft -= 1; renderTimer(); if(secondsLeft <= 0) finishTimer(); },1000);
...
$('finish-btn').addEventListener('click',finishTimer);
