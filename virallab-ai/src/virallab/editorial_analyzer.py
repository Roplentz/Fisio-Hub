...
def _build_payload(structural: Any, semantic: Any, visual: Any, timeline: list[Any]) -> dict[str, Any]:
    moments = []
...
def _from_dict(data: dict[str, Any], engine: str) -> EditorialAnalysis:
    ...
    return EditorialAnalysis(
        engine=engine,
        thesis=text("thesis"), promise=text("promise"), target_audience=text("target_audience"),
        attention_mechanism=text("attention_mechanism"), hook_type=text("hook_type"),
        hook_diagnosis=text("hook_diagnosis"), retention_diagnosis=text("retention_diagnosis"),
        persuasion_diagnosis=text("persuasion_diagnosis"), visual_diagnosis=text("visual_diagnosis"),
        cta_diagnosis=text("cta_diagnosis"), emotional_drivers=[str(x) for x in data.get("emotional_drivers", [])][:6],
        narrative_structure=[dict(x) for x in data.get("narrative_structure", []) if isinstance(x, dict)][:10],
        retention_risks=[dict(x) for x in data.get("retention_risks", []) if isinstance(x, dict)][:8],
        specific_recommendations=[dict(x) for x in data.get("specific_recommendations", []) if isinstance(x, dict)][:10],
        reusable_formula=[str(x) for x in data.get("reusable_formula", [])][:10],
        priority_action=text("priority_action"), executive_summary=text("executive_summary"),
        content_score=score("content_score"), retention_score=score("retention_score"),
        persuasion_score=score("persuasion_score"), clarity_score=score("clarity_score"),
    )
...
def _analyze_locally(payload: dict[str, Any]) -> EditorialAnalysis:
    ...
    if avg_scene > 5:
        risks.append({"moment": "Ao longo do vídeo", "risk": "Planos longos podem reduzir a renovação visual.", "why": f"A média estimada é de {avg_scene:.1f}s por cena."})
    if duration > 90:
        risks.append({"moment": "Metade final", "risk": "Conteúdo longo para consumo vertical sem marcos de progressão claros.", "why": "Vídeos longos precisam reabrir ciclos de curiosidade."})
    if face > 0.75 and framing_changes < 4:
        risks.append({"moment": "Desenvolvimento", "risk": "Excesso de talking head no mesmo plano.", "why": "A presença humana ajuda, mas sem alternância visual pode gerar monotonia."})
    if not cta:
        risks.append({"moment": "Encerramento", "risk": "Ausência de próximo passo explícito.", "why": "O público pode compreender o conteúdo e ainda assim não agir."})

    recs = [
        {"moment": "0–3s", "action": "Transformar a promessa inicial em uma frase visual curta e específica.", "reason": "O início precisa comunicar benefício e tensão antes de qualquer contexto."},
        {"moment": "A cada nova ideia", "action": "Alternar rosto, captura de tela, palavra-chave e exemplo concreto.", "reason": "A mudança deve acompanhar a progressão lógica, não apenas decorar o vídeo."},
        {"moment": "Após 30–45s", "action": "Inserir uma nova pergunta, contraste ou micro-promessa.", "reason": "Isso reabre a curiosidade em conteúdos mais longos."},
    ]
    if not cta:
        recs.append({"moment": "Últimos 5s", "action": "Adicionar um CTA único ligado ao valor entregue.", "reason": "Um próximo passo específico converte atenção em comportamento."})

    formula = ["Contraste ou promessa específica", "Nomeação do problema", "Redução da complexidade", "Sequência prática", "Exemplo ou demonstração", "Síntese", "CTA coerente"]
    content_score = min(100, max(35, round((payload["format_score"] + 70 + (80 if thesis else 45)) / 3)))
    retention_score = min(100, max(20, round((payload["pacing_score"] + (80 if avg_scene <= 4 else 45) + min(framing_changes * 8, 80)) / 3)))
    ...
    return EditorialAnalysis(
        engine="local_editorial_fallback", thesis=thesis or "A tese não ficou explícita na transcrição.",
        promise=promise, target_audience="Público interessado no tema apresentado, não explicitado com precisão.",
        ...
        cta_diagnosis=f"CTA identificado: {cta}" if cta else "Não foi identificado um CTA claro no encerramento.",
...
def _local_structure(payload: dict[str, Any]) -> list[dict[str, str]]:
    ...
    if timeline:
        return [{"time": f"{x['start']:.0f}–{x['end']:.0f}s", "function": x["narrative_label"] or "Desenvolvimento", "content": x["speech"][:220], "visual": x["visual_role"] or x["shot_type"]} for x in timeline[:8]]
    duration = payload["duration_seconds"]
    return [
        {"time": f"0–{max(3, round(duration * .08))}s", "function": "Hook", "content": payload["hook_text"], "visual": "Abrir com mudança ou texto de impacto"},
        {"time": f"{round(duration * .08)}–{round(duration * .7)}s", "function": "Desenvolvimento", "content": "Explicação principal", "visual": "Alternar rosto e evidências"},
        {"time": f"{round(duration * .7)}–{round(duration)}s", "function": "Síntese e CTA", "content": payload["cta_text"] or "Encerramento sem CTA explícito", "visual": "Retorno ao apresentador"},
    ]
