...
def _run(command: list[str]) -> subprocess.CompletedProcess[str]:
    try:
        ...
        raise VideoAnalysisError(
            "FFmpeg/FFprobe não está disponível neste ambiente."
        ) from exc
...
def _detect_cuts(
    path: Path, threshold: float = 0.30, max_seconds: float = 120.0
) -> list[float]:
...
def analyze_video(path: str | Path, scene_threshold: float = 0.30) -> VideoAnalysis:
    ...
    if not isinstance(video_stream, dict):
        raise VideoAnalysisError(
            "O arquivo enviado não contém uma faixa de vídeo válida."
        )
    ...
    orientation = (
        "vertical" if height > width else "horizontal" if width > height else "quadrado"
    )
    fps = _parse_fps(
        str(
    ...
    format_score = _clamp(
        95
        if orientation == "vertical" and 0.50 <= ratio <= 0.67
        else 70
        if orientation == "quadrado"
        else 42
    )
    technical_score = _clamp(
        50
        + (15 if width >= 1080 else 8 if width >= 720 else 0)
        + (12 if fps >= 29 else 5)
        + (12 if audio_stream else 0)
    )
    viral_score = _clamp(
        hook_score * 0.30
        + pacing_score * 0.30
    ...
    if orientation == "vertical":
        ...
        improvements.append(
            "Adaptar o enquadramento para 9:16 para melhorar o uso em redes verticais."
        )
    if early_cuts >= 1:
        strengths.append(
            "Há mudança visual nos primeiros 3 segundos, favorecendo o hook."
        )
    else:
        improvements.append(
            "Inserir uma mudança visual ou texto de impacto nos primeiros 3 segundos."
        )
    if 1.8 <= average_scene <= 5.5:
        strengths.append("O ritmo médio de cenas está dentro de uma faixa dinâmica.")
    elif average_scene > 5.5:
        improvements.append(
            "Reduzir a duração média das cenas ou adicionar B-roll para evitar monotonia."
        )
    else:
        improvements.append(
            "Alguns cortes podem estar rápidos demais; preserve tempo para compreensão."
        )
    if audio_stream:
        strengths.append(
            "O arquivo contém faixa de áudio, permitindo futura transcrição e análise da fala."
        )
    else:
        improvements.append(
            "O vídeo não possui áudio detectável; a análise semântica ficará limitada."
        )
