...
if view == "Início":
    ...
    with left:
        ...
        with upload_tab:
            uploaded = st.file_uploader("Escolha um vídeo", type=["mp4", "mov", "m4v", "webm", "mkv"])
        ...
        with url_tab:
            ...
            if st.button("Importar e analisar URL →", type="primary", use_container_width=True, disabled=not bool(url.strip())):
                try:
                    ...
                    st.error(str(exc))
    with right:
        st.markdown('<div class="action-card"><div class="kicker">CAMINHO 02</div><h3>Criar do zero</h3><p>Comece por tema, público e objetivo.</p></div>', unsafe_allow_html=True)
    ...
    st.markdown('<section class="hero"><div class="kicker">Memória editorial auditável</div><h1>DNA RP</h1><p>Registre escolhas que orientarão as próximas gerações.</p></section>', unsafe_allow_html=True)
