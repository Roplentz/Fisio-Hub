...
st.title("💾 Projetos")
st.caption(
    "Salve, reabra e continue seus roteiros, criativos e renders sem misturar trabalhos."
)
...
if current_package is not None:
    with st.container(border=True):
        ...
        st.caption(
            f"O roteiro aberto será salvo como Projeto {int(current_id):03d}, incluindo os criativos já gerados na pasta atual."
        )
        current_name = st.text_input(
            "Nome do projeto",
            value=getattr(getattr(current_package, "brief", None), "theme", "")
            or f"Projeto {int(current_id):03d}",
            key="save-current-project-name",
        )
        if st.button(
            "💾 Salvar projeto atual", type="primary", use_container_width=True
        ):
            try:
                ...
                if (
                    ...
                    and current_dir.resolve() != destination.resolve()
                ):
                ...
                st.success(
                    f"Projeto {saved.project_id} salvo. Você pode sair e reabri-lo pela biblioteca."
                )
                st.rerun()
...
with st.container(border=True):
    ...
    with left:
        ...
        name = st.text_input(
            "Nome do novo projeto", placeholder="Ex.: IA e prontuários clínicos"
        )
        if st.button(
            "＋ Criar projeto numerado", type="primary", use_container_width=True
        ):
            saved = store.save(
                next_id, name=name or f"Projeto {next_id}", status="novo"
            )
            st.session_state.project_id = saved.project_id
    ...
    with right:
        ...
        if backup is not None and st.button(
            "Importar como novo projeto", use_container_width=True
        ):
            try:
...
st.info(
    "No Streamlit Cloud, o disco pode ser reiniciado em atualizações ou períodos de inatividade. Use **Baixar backup ZIP** para manter uma cópia permanente no celular."
)
        ...
        with st.container(border=True):
            ...
            if a.button(
                ...
                try:
                    ...
                    st.session_state.package_dir = str(
                        store.project_dir(saved.project_id)
                    )
                    st.session_state.preferred_hook = preferred_hook or (
                        package.hook if package else ""
                    )
                    st.session_state.nav_view = "Studio"
                    st.switch_page("app.py")
                except Exception as exc:
            ...
            with st.expander("Gerenciar projeto"):
                confirm = st.checkbox(
                    "Confirmo que desejo excluir", key=f"confirm-{project.project_id}"
                )
                if st.button(
                    "Excluir projeto",
